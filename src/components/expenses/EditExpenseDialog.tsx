...
export const EditExpenseDialog = ({
  ...
  const {
    ...
    defaultValues: {
      ...
      split_percentage_user1: expense.split_percentage_user1,
      split_percentage_user2: expense.split_percentage_user2
    }
  })
  ...
  const onSubmit: SubmitHandler<ExpenseFormData> = async (data) => {
    try {
      ...
      await expensesService.updateExpense(expense.id, {
        ...
        split_percentage_user1: data.split_percentage_user1,
        split_percentage_user2: data.split_percentage_user2
      })

      // Update budget spending if needed
      // If category changed, decrement old and increment new
      if (oldCategory !== data.category) {
        await budgetsService.decrementBudgetSpending(session.couple.id, oldCategory, oldAmount)
        await budgetsService.incrementBudgetSpending(session.couple.id, data.category, newAmount)
      } else if (oldAmount !== newAmount) {
        // If only amount changed, update the difference
        const difference = newAmount - oldAmount
        if (difference > 0) {
          await budgetsService.incrementBudgetSpending(session.couple.id, data.category, difference)
        } else {
          await budgetsService.decrementBudgetSpending(session.couple.id, data.category, Math.abs(difference))
        }
      }

      toast({
        title: 'Expense updated!',
        description: `Updated expense in ${data.category}`
      })
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {receiptFile ? receiptFile.name : expense.receipt_url ? 'Replace receipt' : 'Upload receipt'}
              </Label>
              {receiptFile && (
                <Button
                  type="button"
